...
class CourtsideIQSupabaseUser extends BaseAuthUser {
  CourtsideIQSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> courtsideIQSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CourtsideIQSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
