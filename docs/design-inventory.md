...
| **Birth-date-missing caveat** | `649:2199` | Uncalibrated-rating caveat banner | **Specced only, deliberately dropped** | `birth_date_nudges.dart:5-12` says the banner "IS DELIBERATELY NOT HERE" because "no birth date, no rating" removed the thing it caveated. **Annotated as retired 2026-09-13** (`retired-note` `965:4676`) rather than deleted, so the reasoning survives beside the drawing. |
...
**Divergence resolved 2026-09-13.** `game_setup_pickers.dart:13-16` recorded that the Team
Selection and Event Selection frames used an older "X" close treatment while the code renders
them with the `CiSheet` shell, ending "THE FRAMES SHOULD BE UPDATED, not the code." Both frames
...
| **Profile inline prompt** | **Correct that it does not exist, but it was designed then deliberately killed.** The frame is `Birth-date-missing caveat` `649:2199`. `birth_date_nudges.dart:5-12` says it "IS DELIBERATELY NOT HERE": once "no birth date, no rating" shipped there was no uncalibrated rating left to caveat, and on the Development tab it put the same sentence on screen twice. The ask now lives in the Development tab's empty state. The dead v1.5 `birth_date_profile_banner.dart` was the old jade-coloured version of this; it was unreachable and has been deleted. **Annotated as retired 2026-09-13** (`retired-note` `965:4676`), following the file's own precedent at `801:3265`. Not deleted: the frame is design history and the note explains why it was dropped. |
...
| ~~Team / Event Selection frames are stale~~ | **Fixed 2026-09-13** | `game_setup_pickers.dart:13-16` said the frames used an old "X" close while the code uses the `CiSheet` shell, ending "THE FRAMES SHOULD BE UPDATED, not the code." Both frames now match the shell: `color/border-strong` grabber, Bold 20 title at 24/34, and a real `IconButton` instance with the close glyph. No Save CTA was added: `game_setup_pickers.dart:8-11` commits on tap by design, and the frames were already right about that. |
| ~~What's New sheet frames are unsectioned and mislabelled~~ | **Fixed 2026-09-13** | `892:3184` moved into `1 · Entry & Auth` at the next flow slot (x 8040), section widened to 8490. The "(DRAFT for review)" label became the entry-label "← first launch after updating the app". `899:6` stays outside the flow, correctly, and now carries an `exploration-note` saying so. |
